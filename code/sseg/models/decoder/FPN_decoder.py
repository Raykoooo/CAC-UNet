...
from ..modules.upsample import pad
...
class FPNDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.merge_policy == 'add':
            x = pad(s5, s2) + pad(s4, s2) + pad(s3, s2) + s2
...
@DECODER.register("FPNdecoder")
def build_FPNdecoder(input_channels):
    input_channels.reverse()
    return FPNDecoder(input_channels)